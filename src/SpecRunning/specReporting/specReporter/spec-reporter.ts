...
export class SpecReporter implements ISpecReporter {
  ...
  getStatistic(): SpecReportStatistic{
    ...
    this.specReports.forEach((report) =>{
      stat.count++;

      if(report.isIgnored()){
        stat.ignored++;
      }
      else if(!report.isExecutable()){
        stat.notExecutable++;
      }
      else if(report.isInvalidSpec()){
        stat.invalid++;
      }
      else if(report.isRunFailed()){
        stat.failed++;
      }
      else {
        stat.successful++;
    ...
    stat.topics = this.topicsAndReports.size;
    if(stat.topics>0) stat.topics--;
